  ...
  return (
    <AnimatePresence>
      {enabled && hovered && hoverRect && (
        <motion.div
          key="sb001-hover-bg"
          className="pointer-events-none absolute z-0 rounded-md bg-pink-300/20 transition-all duration-100 ease-out"
          style={{
            right: 0,
            top: hoverRect.top + 2,
...
export const Sidebar001Item = memo(function Sidebar001Item({
  ...
  return (
    <div className="relative">
      {isActive && (
        <span className="pointer-events-none absolute z-10 left-[4px] top-1/2 h-[1.8px] w-[23px] -translate-y-1/2 rounded-full bg-pink-300 transition-all duration-100 ease-out" />
      )}

      <span
        className="pointer-events-none absolute left-0 top-1/2 -translate-y-1/2 h-px bg-foreground/50 transition-all duration-100 ease-out"
        style={{ width: isActive ? 0 : isHovered ? 26 : 18 }}
      />
      <span className="pointer-events-none absolute w-[13px] left-0 top-1/4 h-px bg-foreground/30" />
      <span className="pointer-events-none absolute w-[16px] left-0 top-0 h-px bg-foreground/30" />
      <span className="pointer-events-none absolute w-[13px] left-0 top-3/4 h-px bg-foreground/30" />

      <div
        ref={itemRef}
        className="transition-all duration-100 ease-out"
        style={{ opacity, transform: `translateX(${x}px)`, transformOrigin: "left center" }}
      >
        <a
          href={href}
          onClick={onClick}
        ...
        >
          ...
          {isNew && (
            <span className="size-1.5 rounded-full bg-pink-300 shrink-0" />
          )}
        </a>
      </div>
    </div>
  ...
  return (
    <div className={cn("flex flex-col", className)}>
      <button
        ...
        className="relative z-1 flex items-center gap-1.5 py-1.5 pr-2 pl-4 select-none text-left w-full group"
      >
        {icon ? (
          <>
