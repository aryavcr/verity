    ...
    return (
      ...
      >
        <TabsPrimitive.List
          ...
          className={cn(
            "relative inline-flex items-center gap-0.5 select-none bg-[#d4d7d9]/7 p-1 shadow-surface-4 rounded-xl",
            className,
          )}
          {...props}
        >
          ...
          {selectedRect && (
            <motion.div
              className={cn(
                "absolute pointer-events-none bg-[#d4d7d9]/12 shadow-surface-8 rounded-lg",
              )}
              initial={false}
              animate={{
          ...
          <AnimatePresence>
            {hoverRect && !isHoveringSelected && selectedRect && (
              <motion.div
                className={cn(
                  "absolute pointer-events-none bg-white/20 rounded-lg",
                )}
                initial={{
                  left: selectedRect.left,
                  width: selectedRect.width,
  ...
  ({ value, icon: Icon, label, _index = 0, className, ...props }, ref) => {
    ...
    return (
      <TabsPrimitive.Trigger
        ...
        className={cn(
          "relative z-10 flex items-center gap-2 px-2.5 py-1 cursor-pointer bg-transparent border-none outline-none",
          className,
        )}
        {...props}
      >
        ...
        <span className="inline-grid text-[14px] whitespace-nowrap">
          ...
          <span
            className={cn(
              "col-start-1 tracking-wid row-start-1 transition-[color,font-variation-settings] duration-80",
              isActive ? "text-foreground" : "text-muted-foreground",
            )}
            style={{
