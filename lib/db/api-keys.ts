...
export async function getKeyPresence() {
  ...
  const out: Record<Provider, { present: boolean; last4: string | null }> =
    Object.fromEntries(PROVIDERS.map((p) => [p, { present: false, last4: null }])) as never;
  if (!user) return out;
  const { data } = await supabase.from("user_api_keys").select("provider, last4").eq("user_id", user.id);
  for (const row of data ?? []) {
    if (isProvider(row.provider)) out[row.provider] = { present: true, last4: row.last4 };
  }
  return out;
}
...
export async function saveKey(provider: Provider, plaintext: string) {
  ...
  const { error } = await supabase
    .from("user_api_keys")
    .upsert(
      { user_id: user.id, provider, ciphertext, last4, updated_at: new Date().toISOString() },
...
export async function getDecryptedKey(provider: string): Promise<string | undefined> {
  ...
  try { return decrypt(data.ciphertext); } catch { return undefined; }
}
